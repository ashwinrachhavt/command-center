...
require("@next/env").loadEnvConfig(root, true);
...
try {
  ...
  if (!/^pk_(test|live)_/.test(key) || !/^sk_(test|live)_/.test(secret)) {
    throw new Error(
      "Add the matching Clerk publishable and secret keys to root .env.",
    );
  }
  ...
  const responses = await Promise.all([
    fetch(`${issuer.origin}/.well-known/jwks.json`, {
  ...
  if (responses.some((response) => !response.ok))
    throw new Error("Clerk could not verify the configured key pair.");
  const [publicKeys, privateKeys] = await Promise.all(
    responses.map((response) => response.json()),
  );
  if (
    !publicKeys.keys.some((key) =>
      privateKeys.keys.some(
        (other) => key.kid === other.kid && key.n === other.n,
      ),
    )
  ) {
  ...
  if (process.argv.includes("--sync")) {
    ...
    console.log(
      "Clerk verified. FastAPI issuer synchronized with root Clerk settings. Restart the API after changing keys.",
    );
  } else if (current !== issuer.origin) {
    throw new Error(
      "FastAPI issuer differs from Next.js. Run make auth-sync after changing Clerk credentials.",
    );
  } else {
  ...
  console.error(
    error instanceof Error && !error.message.includes("fetch")
      ? error.message
      : "Clerk verification failed. Check credentials and network access.",
  );
  process.exitCode = 1;
}
