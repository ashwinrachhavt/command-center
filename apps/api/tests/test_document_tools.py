...
def test_document_tools_bound_passages_require_evidence_and_do_not_grant_review(settings, mocker):
    ...
    assert dict(requests[-1].url.params) == {"offset": "12", "limit": "12000"}
    proposal = {
        "field": "skill",
        "value": "Python",
...
def test_escaped_document_passages_keep_valid_json_and_exact_continuation(settings, mocker):
    ...
    def page(method, path, *, params):
        ...
        text = original[offset : offset + params["limit"]]
        return {
    ...
    first = registry.execute("document_read", {"version_id": version_id}, "read-1")
    ...
    second = json.loads(
        registry.execute(
            "document_read", {"version_id": version_id, "offset": parsed["next_offset"]}, "read-2"
        )
    )
    assert parsed["text"] + second["text"] == original
