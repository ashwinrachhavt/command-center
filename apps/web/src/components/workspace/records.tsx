...
export function Records({ resource }: { resource: Resource }) {
  ...
  const select = (id: string | null) =>
    router.replace(`/${resource}${id ? `?record=${id}` : ""}`, {
      scroll: false,
    });
  return (
    <>
      ...
      <div className="border-y border-border">
        <div className="flex flex-wrap items-center gap-3 px-5 py-3 md:px-9">
        ...
        {query.isPending ? (
          ...
          <EmptyState
            title={
              q || filter !== "all"
                ? "No matching records"
                : `Your ${name.plural.toLowerCase()} start here`
            }
            description={
              q || filter !== "all"
                ? "Try a different search or clear your filters."
                : `Add your first ${name.singular} and give your next move a place to start.`
            }
          >
            {q || filter !== "all" ? (
              <Button
            ...
            <TableBody>
              {rows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell>
                    <button
                      className="flex max-w-96 items-center gap-3 text-left"
                  ...
                  <TableCell className="text-xs text-muted-foreground">
                    {resource === "companies" || resource === "jobs" ? (
                      ...
                      dateLabel(field(row, "due_date") || field(row, "due_at"))
                    ) : (
