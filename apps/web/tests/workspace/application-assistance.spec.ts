...
test("pins an explicit resume, replacement, upload field, and preparation version", async ({
  ...
  await page
    .getByRole("checkbox", {
      name: "Upload selected resume to Resume",
    })
    .click();
  await page
  ...
  const reviewedFields = Object.fromEntries(
    state.preparation.fields.map((field: { field_id: string; value: string | null }) => [
      field.field_id,
      field.value,
    ]),
  );
...
test("explicitly clears generated suggestions and replacement opt-outs", async ({
  ...
  await replace.click();
  await page.getByRole("textbox", { name: "Email *" }).fill("skip@example.test");
  await replace.click();
  ...
  expect(values).toMatchObject({ f0: "Synthetic Candidate", f1: null, f2: null });
