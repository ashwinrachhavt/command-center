...
**Home** opens agent conversations. **Opportunities** groups broad opportunities, applications and saved roles; **Tasks** keeps the actionable queue. Agent profiles, connectors, skills and memory are accessible from **Agents**. The older `/overview` route retains Today, Upcoming, Unscheduled and Snoozed groups using your saved timezone. Task status changes remain explicit; snoozing stays paused until you resume.
