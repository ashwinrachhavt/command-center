...
On the isolated `daily-workspace` branch, **Overview** groups tasks into Today, Upcoming, Unscheduled and Snoozed using your saved profile timezone. Separate queues expose questions and reviews, active work and saved outputs. Open a row to continue its conversation, review the exact action or read the saved document version. Task status changes are explicit; snoozing stays paused until you resume. These changes are not yet in the running `frontend-redesign` smoke-test build.
