...
Durable connected-request claims retain one provider session per run/account/auth-config/policy revision, including resumed runs. Manual pulls use their request scope. Retries reuse the ID and never fall back to unrestricted execution. Failed or ambiguous session creation is not silently replayed. Account identity is verified before reads and selection/identity/lease rechecked before saving an observation. `request_message_id` must identify the current run's owned human input. This checks provenance, not natural-language meaning: explicit-mail intent is enforced by the agent instructions, not a semantic server classifier. Ordinary drafting and retrieved instructions must not initiate mail reads.
