...
`GET /artifacts` retains `collection=all` for existing consumers. `library`, `notes` and `generated` exclude imported originals and linked extraction artifacts. `vault` includes imported originals and groups their extraction through the existing `DocumentImport` linkage. `generated` additionally requires the owner's `artifact.version_created` audit provenance with an `agent_run_id`. Filtering and counts happen before pagination. No duplicate document store or lifecycle is introduced.

The optional `review` filter and `ArtifactRead.review_status` use the most recent review of the latest immutable version, with `unreviewed` when none exists. A review of an older version never approves a replacement. Existing exact-version reading/review/derivation contracts remain unchanged. `/documents` and `/agent-settings` are UI destinations. Legacy routes remain available. Artifact inspection uses the existing URL context stack in a larger modal canvas with focus restoration.

Companion 0.4.6 defaults an unset `pageReader` to `direct`, preserves explicit stored settings, and reports active-tab permission failures with recovery instructions. Native capture still requires explicit selection and never silently falls back. Capture, preparation, exact file review and apply authorization contracts remain unchanged.


## One-click application companion — 2026-09-22

The MV3 companion opens a persistent Chrome side panel. Autofill retains snapshot/preparation/authorization/command identities before I/O, resumes unfinished requests and stores the command result before reporting it. A fill command is claimed before its field changes. Unknown execution never replays; a lost result acknowledgement retries only reporting. Manual edits are frozen during an operation; subsequent reviewed fills recapture the page and require an identical control structure, preserving newly present values.
...
This contract was verified at isolated commit `d83c6d7`, now an ancestor of `application-autofill`; it is not deployed to the user's stable `frontend-redesign` smoke-test build at `2ad39e7` with extension `0.4.3`. Non-browser validation is complete; Engineering records the evidence. Browser QA remains skipped at the user's request.
...
Native date/month controls use validated calendar ranges and exact step checks. Split year/month controls and declared text formats use only known source precision; no missing month/day or current status is inferred. Unknown group components never fall back to unrelated scalar profile facts. Companion `0.4.2` introduced grouped filling without permission or SQL schema changes. Bounded row expansion is specified below; broader ATS compatibility and automatic job identity detection remain open.

### Career row expansion — isolated contract

Branch `application-autofill` in `.local/worktrees/application-keywords` adds companion `0.4.4` over keyword ancestor `d83c6d7`. It is not deployed: the running smoke-test build remains `frontend-redesign` at `2ad39e7` / `0.4.3`. Non-browser verification is complete; browser QA is skipped at the user's request. Native AgentBrowser and real-browser integration were not exercised for this increment.

Preparation creation snapshots `history_targets` with `experience` and `education` counts in `0..10`. Counts reuse autofill's exact deduplication and chronology rules over the owner's active, unexpired, approved, unscoped structured facts. No eligible facts or ambiguous start-date ordering yields zero for that kind. Targets remain unchanged across package revisions; historical payloads read as zero. No endpoint, migration, profile mutation or legacy receipt-hash change is required.

Expansion clicks only explicit Add buttons in an unambiguous history section; occupied groups and ambiguous DOM remain manual. A retained row-operation ID prevents duplicate additions on retries. A rerendered Add button is accepted only after an added row is confirmed and the replacement preserves the history kind, label and section scope. After expansion, a fresh capture and preparation continue the same application before authorization and filling. A history-only page may begin with zero fields via local `InspectResult.history_expandable`; AgentBrowser accepts a zero-field capture only with a valid local Add hint. This marker is stripped before API payloads. Next and Submit remain manual. This bounded contract does not establish authenticated portal coverage or exact Simplify parity.
...
Document imports preserve original PDF/DOCX/JPEG/PNG/text/Markdown bytes in private content-addressed storage and append immutable artifact versions. JPEG/PNG validation derives media type from decoded bytes, checks matching extensions, rejects corrupt/animated images and caps dimensions at 25 megapixels alongside the shared 20 MiB upload limit. Validation runs outside the async request loop. Migration `0006_documents` adds a leased conversion job and an exact default-resume version reference. The Celery worker submits bytes to local Docling, bounds conversion time/results and creates an immutable derived artifact containing text, structured document data and producer version. Images use the image pipeline with full-page OCR; image descriptions and embedded image exports stay disabled. An image placeholder without readable text is not a successful extraction. Duplicate delivery, cancellation and lease expiry cannot publish another output. Every import records acquisition and an open review task. Original downloads require owned artifact/version IDs. A new original never silently replaces the selected default resume.
...
The isolated `application-identity` companion adds nullable `JobIdentity` to preparation create/read and application/history projections. Domain code re-parses the canonical URL, validates its provider/organization/posting tuple, and binds it to the captured path. Greenhouse’s fixed embed route carries only its validated `for`/numeric `token` identity because snapshots deliberately omit query data; other unknown paths cannot claim an arbitrary known posting. Full URLs and query tracking are not added to durable records. Known current/previous identities must agree even when continuation is explicitly requested; otherwise creation conflicts. Equal identified jobs may continue a changed path without the manual new-page flag, recording `same_job`. Unknown pages retain existing confirmation rules and carry the last identity. No migration or duplicate job store is introduced: immutable preparation packages own the observed metadata. Omitted/null metadata leaves legacy request fingerprints unchanged.

Both fixed readers and the domain parser accept only bounded HTTPS known-host/path shapes, reject credentials/nonstandard ports/unsafe path components and preserve explicit organization scope. Workday’s scope includes its full host and recruiting site. Canonical links omit query and fragment data. The popup freezes identity with its captured operation and carries it through manual retries and row recapture; device, owner, task, selected-file and exact page guards remain. Reopening an earlier artifact version preserves its original identity.
...
Implemented on isolated `daily-workspace`: human-only `GET /api/v1/dashboard/work` returns actor-scoped, bounded pages for `attention`, `running` and `outputs`. A shared SQL union supplies rows and counts, ordered by timestamp, kind and ID. Attention includes current open questions, the latest unresolved conversation run, reviewed-action states requiring attention and browser commands needing manual review. Pending expired commands are omitted; a claimed expired command keeps its stored state with an explicit unknown-outcome explanation. GET never expires commands, resumes runs or calls providers. Newer runs, including archived ones, suppress obsolete failed-run attention. Artifact output rows deduplicate task links, pin the latest version of active document/message/research artifacts and expose that exact version’s review state. Saved text is not proof of an external send.

`GET /api/v1/dashboard/tasks` returns the same bounded owned task predicates for rows and per-view counts. The validated IANA timezone establishes local calendar-day bounds using ZoneInfo, including DST; date-only deadlines remain due through that date, while elapsed instant deadlines are overdue. Open/in-progress tasks occupy today/upcoming/unscheduled; snoozed tasks remain separate and done/cancelled are excluded. Results sort by deadline, priority and ID. Overview uses the profile timezone, refreshes saved work state, preserves existing request identities/version checks for explicit task changes and lazily loads the shared review/run UI only when opened. These endpoints add no storage or migrations.
...
LinkedIn connector (2026-09-23): pin toolkit `20260915_00`; managed OAuth requests `openid,profile,w_member_social,email`. `LINKEDIN_WHO_AM_I` verifies the stable OIDC subject and supplies basic profile context. `LINKEDIN_GET_POST_CONTENT` reads an explicit post URN if the provider grants access. `LINKEDIN_CREATE_LINKED_IN_POST` is reachable only through an approved `linkedin_post` revision, with author derived from the verified account, exact text and PUBLIC/CONNECTIONS audience. These extend `connected_context` and `propose_connected_action` for chat and local MCP, preserving ownership, spending reservations, bounded observations and no automatic write replay. Migration 0032 extends constraints and replaces only app-managed default rate cards with new immutable cards containing local LinkedIn estimates; policy limits and prior snapshots are preserved. Additional Composio LinkedIn tools are not exposed by this slice. Personal WhatsApp remains deferred.
...
**Request-cost maintenance, 2026-09-23:** optional local Langfuse traces runs, model calls, tools, cache usage and terminal/pause states with optional bounded, redacted content capture. The lead exposes three discovery/question schemas while retaining its server grants. DeepEval exports synthetic recorded-output scores to request traces and records judge costs separately; a three-case chat smoke is distinct from the full workflow suite. The run-steps projection excludes internal checkpoint fields, and a durable question interrupt remains pending rather than emitting a tool failure. See [request costs and evaluation](request-costs.md) for implemented commands, accounting limits and the optional Jev capability router.
...
### Researched contact connection notes — 2026-09-22

Migration `0027_contact_research` pins `RecordWork.research_requested`. Existing ordinary drafting remains compatible. Research requests require public search, Firecrawl-backed capture, saved-document read and approved-profile tools. Structured `ContactResearch` is embedded in the same immutable follow-up version as the message; no second document lifecycle or automatic CRM overwrite is introduced. Matched identity requires captured identity evidence; current company/role requires matched identity plus employment evidence. Each quote must occur in an owned public source attached to this task and appear in the output derivation sources. This validates provenance/quotation, not semantic truth. Uncertain results carry explicit caveats and omit unsupported employment fields.

Contacts list/detail returns a bounded `outreach` projection using batch queries: latest requested job state plus the last useful LinkedIn note, including subsequent human versions, research and source timestamps. Failed refreshes preserve the previous note. Human checkpoints retain research context and enforce the connection-note 200-character limit, counting UTF-16 units conservatively. The existing receipt, owner, task-scope and lease fencing apply to all writes. Research work uses the configured outreach model with a larger step allowance; no model-provider migration is implied.

### Quick connection notes and explicit enrichment — 2026-09-23

Migration `0029_connection_notes` pins the quick-note contract separately from `research_requested`, defaulting existing records to false. Quick contact requests select the `connection` profile; full contact research selects `outreach`, and company work retains `research`. The quick profile retains the configured supported model (`gpt-6-luna`) with low reasoning, an 800-token output ceiling per model call, seven model steps, eight tool calls, a 40,000-character context ceiling and a 180-second run deadline. Framework instructions/tool schemas count toward context; this is a safety ceiling, not a target prompt size. No research/writing skills are loaded. Saved contact context is compact and may reuse dated research with its caveats.

Per-tool quotas enforce at most one search, capture, document read and approved-profile read in a quick run. Denied attempts are recorded, consume the global tool budget and stay denied across checkpoint replay. Saving and editing still enforce the 200-character contract. Batch enrichment reuses human-only, receipt-protected per-contact start endpoints; the UI selects at most ten contacts and runs two enqueue requests concurrently, retaining uncertain request keys for retries. This does not combine model billing or change worker concurrency.

Usage events preserve strictly validated non-negative integer input/output/total token counts. Generic token-shaped fields remain redacted; previously redacted historical counts cannot support a retrospective cost benchmark. No live latency or cost reduction is claimed by deterministic tests.

## Shared FastMCP and bounded chat context — 2026-09-23

FastMCP 3 hosts the existing `/mcp/` endpoint through request-specific providers over `ToolRegistry`. Every FastAPI operation is classified in `agents/mcp_policies.json`; unclassified operations receive no capability. Run/lease credentials remain separate from revocable actor-bound local MCP credentials and their short-lived API audience. Local clients cannot acquire human-review authority. The supervisor uses explicit `catalog_search`/`catalog_execute` grants to discover bounded schema pages; specialists retain their pinned grants. See [local client setup and policy contract](local-mcp.md).

`POST /api/v1/leads/intake` is an owner-scoped transaction with durable request receipts and serialized intake identity checks. It accepts exactly one literal text source, owned immutable text version or saved user message. Company/contact/opportunity creation, optional job creation and private source evidence commit together. Strong contact identities and company matches are reused without overwriting manual fields; conflicting matches return actionable conflicts. Source message references avoid echoing long pasted content through tool arguments.

Sessions expose recent-activity pagination/search and incremental sequenced messages. Bounded Deep Agents summarization middleware respects the application's character budget, retains authoritative work context and persists a prefix summary with covered-message references; original SQL transcripts remain intact. The summary model call is accounted as model usage. Run-specific checkpoints retain lease and interrupt isolation. Answer reuse is fail-closed: the eligible first text-transformation exchange and its immediate exact repeat, same actor/session/configuration/context, zero observed tools, five-minute expiry and explicit fresh-answer bypass. Persisted provenance explains hits; this does not cache mutable retrieval or tool effects.

## Focused-work prompt and eval contracts — 2026-09-23

The general supervisor loads the scoped `opportunity-work` skill for career tasks. It defines explicit source-tier handling, requested batch size, completion state and next actions without changing CRM urgency or implementing a sorter. Research gains the read-only `approved_profile` tool and a dossier output contract; outreach gains relationship-specific asks and recent-reply handling; application and shared writing instructions require supported fit, finished copyable content and separate gaps. Existing connection-note limits, human-edit preservation, source-version citations, manual submission and reviewed external actions remain in force. Prompts are guidance; models/tool contracts enforce authority and persistence.

`agents/profiles.toml` continues to pin grants and file-backed directives/skills. Config content hashes change with these edits; existing runs retain their snapshots. Compose mounts the agent files, and newly created runs load their current validated configuration. No new runtime, migration, connector or UI flow is introduced.

`apps/api/evals/productivity_cases.json` adds seven entirely synthetic development scenarios to the original seven. `dataset.py` binds their evidence and suite revision `2026-09-23.1`; existing capture/plan digests become stale. Offline tests derive suite and judge-call counts while retaining independent synthetic cost arithmetic and missing-platform checks. The grounding/relevance/completion rubric now assesses source authority, requested mode, relationship, partial completion and duplicate work. There are 42 judge calls per full suite under the current three metrics. Paid execution remains separate and requires a reviewed positive allowance. Reference examples are not model captures. A prompt-only comparison must hold tool grants constant; the research profile's additional read tool otherwise makes this a configuration comparison.

## Scoped Composio mail sessions and document discovery — 2026-09-23

The Gmail search endpoint uses the installed Composio SDK Sessions API (`tool_router.session.create/execute`). Session creation pins the application user, selected verified Gmail account, auth config and only `GMAIL_FETCH_EMAILS`. Connection management, workbench/proxy and multi-execution are disabled. The existing small Command Center catalog exposes the typed read operation; raw provider catalogs and helper tools are not injected into prompts. Sessions use current provider tool contracts; legacy reviewed writes retain their existing version pins and review lifecycle.

Session execution relies on that single pinned account and omits the per-call `account` selector, which Composio rejects when project multi-account support is disabled. Agent-facing direct and progressive-catalog schemas require a non-null `request_message_id`; human and local-client pulls retain their optional field. Missing agent provenance is rejected before provider dispatch with guidance to retrieve `record_work_context.input_user_message_id`. API ownership and consumed-input checks still apply. Gmail tool errors distinguish request authorization from provider/configuration failure without relaying provider payloads or attributing every 503 to a worker outage.

Durable connected-request claims retain one provider session per run/account/auth-config/policy revision, including resumed runs. Manual pulls use their request scope. Retries reuse the ID and never fall back to unrestricted execution. Failed or ambiguous session creation is not silently replayed. Account identity is verified before reads and selection/identity/lease rechecked before saving an observation. `request_message_id` must identify owned human input attached to the current run, from its original input through its persisted consumed sequence. This includes consumed steering instructions while excluding unread future messages and messages from other runs. This checks provenance, not natural-language meaning: explicit-mail intent is enforced by the agent instructions, not a semantic server classifier. Ordinary drafting and retrieved instructions must not initiate mail reads.

Session creation and each Gmail tool execution have distinct spending reservations. Migration `0033_gmail_sessions` adds conservative $0.01 estimates to new immutable app-managed default cards, preserving limits, existing rates, custom cards and historical reservations. Custom cards need `COMPOSIO_SESSION_CREATE` and `GMAIL_FETCH_EMAILS`; missing bounds deny dispatch. Default searches return five messages with bounded fields and explicit per-message truncation. Narrow the query/result count when more source text is needed. Mail content is excluded when telemetry content capture is off; the explicit content-capture setting exports bounded, redacted diagnostic copies.

Document discovery recognizes Document Vault, recent uploads and content queries. Agents list imports newest first, inspect conversion status and read bounded passages using the exact `extraction_version_id`. Upload dialogs accept multiple files for new documents, retain an independent idempotency key per file, upload sequentially and keep successful rows while retrying failures. Titles and document type are fixed after dispatch so retries preserve request identity. Appending to an existing document remains one file/version per action.
