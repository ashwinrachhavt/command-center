...
**Revision:** 2026-09-21-r13. **Status:** connected workspace, operator importer, Deep Agents conversations, lead discovery/evidence/drafts, local document ingestion and reviewed candidate facts implemented locally. Deep Agents supports OpenAI, Gemini, Mistral and Cohere per profile. Reviewed external actions and complete application workflows remain in progress or planned.
...
**Confirmed user decision, 2026-09-21:** author and run tests in **pytest**, use **pytest-mock** (`mocker`) for mocks/spies, and write agent/LLM evals in **DeepEval**. This selects tooling; it does not claim DeepEval has been installed, configured or run.
...
- **Current versus planned:** pytest and pytest-mock are installed for backend tests. Vitest and TypeScript Playwright currently cover frontend components, navigation and the extension; maintain these regression checks while planning any pytest-based browser consolidation. DeepEval dependency/version, suite location, exact metrics/judge, thresholds, CI policy and a bounded paid run allowance remain implementation-review items. No suite migration or eval installation is included in this documentation change.
