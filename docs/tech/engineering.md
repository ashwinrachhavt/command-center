...
- [ ] Application automation and broader Simplify parity: assisted autofill/native AgentBrowser, tracking, saved job descriptions and tailored document generation/editing/export are implemented; deterministic keyword coverage has completed non-browser validation in isolated `application-keywords`. Automatic work/education row creation, broader ATS controls, automatic job identity detection, portal reconciliation and full tenant coverage remain.
...
Implemented a deterministic literal matcher, owner-bound read-only API and Application documents coverage panel in `.local/worktrees/application-keywords` on branch `application-keywords`. The user's stable smoke-test build remains `frontend-redesign` at `2ad39e7`, with extension `0.4.3`; this branch has not been pushed or deployed over that build. No migration, companion update, runtime restart, browser/provider call or model work is part of this pass.
...
Gmail reads are now human-only at the API boundary, including legacy agent grants; active agent profiles no longer grant `gmail_search`. The Reviewed actions page exposes an explicit Pull email dialog, with query, selected account and up to ten results. Opening it or entering a query cannot fetch messages. A changed selected account rejects the pull. Gmail remains the inbox; no mailbox sync or new routines were added. True thread/reply targeting remains pending.
