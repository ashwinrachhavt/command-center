...
The local companion now provides a persistent side panel and one-click Autofill: inspect the current page, derive answers from approved facts, attach the selected exact résumé, and show per-field results and missing questions. A native helper invokes AgentBrowser in a dedicated browser profile, bound to the requested full URL and a per-inspection nonce. Direct browser capture is a separately selected option. Edited answers can be saved and filled after a fresh capture, retaining the same application task on the same page. Unknown facts, ambiguous widgets and newly edited page values remain visible rather than guessed or overwritten. This is the first complete local Copilot flow, not universal Simplify parity; the comparison below remains a delivery checklist.
...
| Employment and education | Structured editing, review and matching to existing explicit groups implemented; automatic row creation and broader ATS coverage remain |
| Generated narrative answers | Existing grounded draft jobs, editable review and fill; paid model quality remains unverified |
| Saved application work | Canonical task, immutable preparations and per-field command outcomes; repeat captures of the same application can reuse the task |
...
| Résumé tailoring, keyword matching and scoring | Job-specific drafting, editing, PDF export and explicit résumé selection implemented; keyword matching/scoring remain |
