...
| Résumé tailoring, keyword matching and scoring | Drafting, editing, PDF export and résumé selection implemented; deterministic keyword coverage has completed non-browser validation in isolated `application-keywords`, separate from the stable smoke-test build |
