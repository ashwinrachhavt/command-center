...
**Revision:** 2026-09-21-r9. **Status:** connected workspace and operator importer implemented; accepted hardening and future workflow contracts remain explicit. Deep Agents on LangGraph selected for the new agent architecture; detailed design and engineering review in progress.
...
The implemented stack is FastAPI, SQLAlchemy/Alembic, PostgreSQL 17 in Compose, Next.js/React/TypeScript, Clerk, custom LangGraph/OpenAI, Celery/Redis and an initial Composio integration. The user selected Deep Agents on LangGraph for the next architecture and made Composio central to external integrations. Exact installed dependencies live in `apps/api/uv.lock` and `apps/web/package-lock.json`; Deep Agents is not installed yet. Reuse the existing Docker Firecrawl/SearXNG services. PostgreSQL supersedes the historical SQLite/FTS5 proposal; Comp AI remains a reference, not a fork.
...
The implemented shadcn workspace exposes actor-owned CRM, profile, tasks, artifacts/versions/reviews and audit. Clerk session JWTs are verified against the configured issuer/JWKS and authorized origins. The browser calls a same-origin Next.js forwarding route with the verified session token. User-provided actor IDs never grant access; reads and foreign references are actor-scoped. Provider secrets stay in ignored root `.env` and `apps/web/.env`, outside client bundles, prompts and memory. The diagnostic bearer credential is separate from human Clerk identity and signs scoped internal run capabilities.

Receipt-backed mutations persist an actor-scoped request hash/result with the model change and reject reuse of an Idempotency-Key with different input. New row UUIDs derive from actor, resource and key. Mutable records use optimistic revisions and reject stale edits. The accepted frontend work to preserve those keys, drafts and identity boundaries is tracked in [engineering.md](tech/engineering.md#accepted-workspace-hardening); server guarantees do not establish correct client retry behavior. Provider connection initiation has no equivalent local receipt guarantee.

Alembic owns one schema history. Separate runtime/migration URLs and session/transaction pool settings support a later Supabase deployment while retaining FastAPI as the access boundary. Application tables have RLS with no public Data API policies. Supabase deployment and validated backup/restore are not completed. Root [README](../README.md) owns setup and migration commands.
...
| `agents/config.py` eagerly concatenates up to ten Markdown skills; combined instructions can exceed the 20,000-character profile limit and fail worker revalidation | Validate the effective snapshot at enqueue time and move to supported progressive skill loading; current hashing alone does not bound the assembled context |
...
| Questions and resume requests | Proposed durable user-input records | Bind each question to owner, run, interrupt ID and revision. Accept a response once; a duplicate response cannot resume a later question or unrelated checkpoint |
...
Proposed concurrency rule: serialize root graph continuations for one session/checkpoint thread while allowing independent sessions and safe child reads to run concurrently. Persist incoming user messages before processing. The exact behavior for a message arriving during active execution—steer at a safe boundary or queue a later turn—remains an interaction detail to settle. Never start two root writers on the same checkpoint merely because the user opened two tabs. Session context survives refresh/restart; reconnecting a browser cannot revive an expired form command.
...
The current `memory_append` grant does not enforce this new approval lifecycle. The migration must expose a proposal operation to agents and enforce promotion in domain models, not rely on prompt wording. Preserve existing note provenance and explicitly classify legacy notes rather than silently representing old agent-written text as newly reviewed. Ordinary task history, source captures, scratch artifacts and conversation compaction are execution records/context; saving them does not make them reusable approved memory.
...
- **Outreach:** on the user's request, search relevant messages in one selected Gmail account (AR-17) alongside permitted person/company/task context, save a typed email draft and show sender/recipient/subject/body/attachments in Command Center. Approval binds that version and account. The executor sends once when verifiable and records the receipt; an ambiguous send remains unresolved until reconciled. Editing after approval requires a new review. No background inbox monitoring or multi-account search is part of the first path.
- **Research:** the lead delegates a source-backed company/interview brief, the specialist uses research tools and optional isolated scripts, and saves the document with sources and derivations. The work continues with the laptop closed. Notion publication is a separate reviewed action; a script or publication failure preserves the saved research.

Before implementation, review the data constraints, checkpoint/ledger crash boundaries, action executor, browser coverage and sandbox limits. Then convert the agreed design into vertical slices in [Engineering](tech/engineering.md); this draft does not mark those checks or the full plan-eng-review complete.
...
Before claiming a platform supported, record the tested form/control variants, repeatable synthetic fixtures, upload behavior, dynamic-field behavior and refreshed-page continuation. Site identity alone is insufficient proof that a particular employer form can be handled. Exact supported controls, iframe/custom-widget behavior, control-specific enforcement of existing-value preservation and representative examples remain to be specified and verified. Unsupported controls must stay visible to the user rather than being reported as completed. [Engineering](tech/engineering.md#agent-release-verification-draft) owns the coverage matrix and evidence.
...
The following is the recorded 2026-09-21 research snapshot, not a provider/account execution verification. Simplify's public Copilot interface and official help were rechecked during the new agent interview; the [reference study](tech/design.md#simplify-copilot-reference--agent-interview) distinguishes Copilot assistance from Autopilot submission and records ambiguous multi-page wording. Product direction selects AgentBrowser and Docling; neither autonomous AgentBrowser orchestration nor Docling extraction is integrated into the current application. The local manual-fill companion is already implemented. Revalidate external capabilities when implementing the relevant slice.
...
Resolve the remaining [Product Spec decisions](product-spec.md#7-open-product-decisions--next-interview-round) for the workflow they govern. Stack, Clerk identity, revocable companion authentication, manual-fill contract, LangGraph/Celery runtime, artifact/version structure and model/controller style are implemented. AR-15 selects Deep Agents on LangGraph for the new architecture; the current runtime remains the baseline until the reviewed migration is built. Other established decisions remain in force.

Still open: detailed fact/answer provenance and reuse, field/tenant coverage within the five selected application platforms, resume ingestion/version implementation, account configuration, numeric budgets, detailed dashboard layout, active-turn steering, byte storage/retention, sandbox provider/limits and always-on hosting/recovery targets. AR-17 fixes relevant Gmail searches on request in one account; AR-18 fixes work-first dashboard priority; AR-19 fixes task/opportunity conversation scope; AR-20 requires review of proposed long-term memories; AR-21 preserves existing application values; AR-22 selects the general portfolio resume by default; AR-23 accepts spending-control mechanics without setting dollar amounts. Later campaign authority, autonomous navigation/submission and Slack remain separate. Accepted workspace hardening and the UI simplification review live in the supporting engineering/design documents. A completed review is not proof that its implementation tasks have shipped.
