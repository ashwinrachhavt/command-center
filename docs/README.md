...
The latest application checkpoint adds saved job descriptions, recoverable tailored résumé/cover-letter requests, contextual document editing and PDF export, plus separate résumé/cover-letter selection and exact file attachment for autofill. See [Engineering](tech/engineering.md#application-materials--current-pass) for validation and deployment status. Settings also supports structured employment/education review with recoverable Tiptap descriptions. Existing explicit employment/education groups now support approved-fact autofill with date precision and whole-group preservation. The companion also retains confirmed later pages under one application and preserves the choice across interrupted requests. Deterministic keyword coverage with optional selected terms and exact source versions was verified at isolated ancestor `d83c6d7`, now included in `application-autofill`; see [Engineering](tech/engineering.md#application-keyword-coverage--isolated-branch-verified). It is a read-only text comparison with no model calls or fact/status changes, not an ATS ranking or eligibility decision. This feature is not in the user's stable smoke-test build, `frontend-redesign` at `2ad39e7` with extension `0.4.3`. Bounded career row expansion is implemented in isolated `application-autofill` / companion `0.4.4`, using saved approved-fact targets and explicit Add controls; [non-browser verification is complete](tech/engineering.md#career-row-expansion--isolated-branch-non-browser-verified), browser QA is skipped at the user’s request, and it is not deployed. Recognized current-application posting identity and automatic same-job page continuity are implemented on isolated `application-identity` / companion `0.4.5`, with non-browser verification complete; see [Engineering](tech/engineering.md#recognized-application-identity--isolated-implementation). Historical-application matching, broader ATS coverage and exact Simplify parity remain unfinished. Paid evaluation remains disabled until a reviewed plan and allowance exist. The consolidated attention/running/output projection and daily-task UI are implemented on local `daily-workspace`, with non-browser verification complete; see [Engineering](tech/engineering.md#daily-workspace--isolated-implementation). They are not in the stable smoke-test build. Hands-on platform/provider QA is deferred to the user.
