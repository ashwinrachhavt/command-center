...
The latest application checkpoint adds saved job descriptions, recoverable tailored résumé/cover-letter requests, contextual document editing and PDF export, plus separate résumé/cover-letter selection and exact file attachment for autofill. See [Engineering](tech/engineering.md#application-materials--current-pass) for validation and deployment status. Settings also supports structured employment/education review with recoverable Tiptap descriptions. Existing explicit employment/education groups now support approved-fact autofill with date precision and whole-group preservation. The companion also retains confirmed later pages under one application and preserves the choice across interrupted requests. Deterministic keyword coverage with optional selected terms and exact source versions is implemented in isolated `application-keywords`, with non-browser validation complete; see [Engineering](tech/engineering.md#application-keyword-coverage--isolated-branch-verified). It is a read-only text comparison with no model calls or fact/status changes, not an ATS ranking or eligibility decision. This feature is not in the user's stable smoke-test build, `frontend-redesign` at `2ad39e7` with extension `0.4.3`. Automatic row creation, automatic job identity detection and broader ATS compatibility remain unfinished. Paid evaluation remains disabled until a reviewed plan and allowance exist. The overview's consolidated attention/running-work/output projection and daily-task polish remain unfinished; hands-on platform/provider QA is deferred to the user.
